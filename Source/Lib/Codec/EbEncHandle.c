...
void EbSetThreadManagementParameters(EB_H265_ENC_CONFIGURATION   *configPtr){
    ...
    if (numGroups == 1) {
        ...
        if (configPtr->logicalProcessors == 0) {
            if (configPtr->targetSocket != -1) {
                for(int unsigned i=0; i<lpgroup[configPtr->targetSocket].num; i++)
            ...
            if (configPtr->targetSocket == -1) {
                ...
                if(lps > numLpPerGroup) {
                    for(int unsigned i=0; i<lpgroup[0].num; i++)
                        CPU_SET(lpgroup[0].group[i], &groupAffinity);
                    for(int unsigned i=0; i< (lps -lpgroup[0].num); i++)
...
)
{
    ...
    sequenceControlSetPtr->totalProcessInitCount += sequenceControlSetPtr->encDecProcessInitCount                       = 1; //MAX(40, coreCount);
...
void CopyApiFromApp(
    ...
    sequenceControlSetPtr->staticConfig.colorFormat = ((EB_H265_ENC_CONFIGURATION*)pComponentParameterStructure)->colorFormat;
    if (sequenceControlSetPtr->staticConfig.colorFormat == EB_YUV400) {
        SVT_LOG("Color format EB_YUV400 not supported, set to EB_YUV420\n");
        sequenceControlSetPtr->staticConfig.colorFormat = EB_YUV420;
    }
    sequenceControlSetPtr->chromaFormatIdc = (EB_U32)(sequenceControlSetPtr->staticConfig.colorFormat);
    ...
    EB_U8                       *src)
{
    ...
    EB_U16                           colorFormat = (EB_U16)(config->colorFormat);
    ...
    EB_BUFFERHEADERTYPE        *inputBuffer)
{
    ...
    inputPictureBufferDescInitData.colorFormat = (EB_COLOR_FORMAT)config->colorFormat;
